...
| push pop | not migrated | hand (50+r / 58+r, REX.B) | no |
| nop | not migrated | hand (90) | no |
| .frame | not migrated | hand (alu_imm on rsp; REGMAP x86 r7) | no |
...
| gate (syscalls) | not migrated | the abi table (sysno, arg regs, gate form) plus hand | **yes** on win (imports), and cells in data |
| argsave, argvget, spinit | not migrated | hand (SysV/Mach-O entry) | **yes**: data cells |
| winsave, winrest, winargs, winstdh | not migrated | hand | **yes**: win data and imports |

**What remains without a layout dependency:** push/pop, nop, .frame, the four divisions, .zero, the FP group, and setreg for imm/reg. These can be done as fixture slices exactly as so far.
