...
|---|---|---|---|
| **P0 弃出货 Python** | `web-build` / `ujs2wasm` CLI / 预编译 embed **不再是 ship 必经** | ① Pages 玩法步进只加载直出 `.wasm`（B），不再 `wasm_run`+bytecode ② 改 `.ujs` 用 **JS/wasm 宿主**重编译，而不是 `python3 -m ujs …` ③ `npm run ship:*` 里无 `python3` | Asteroid+drone **M1/M1b✓**；**M2✓**；**M3 v10✓**（sim.ujs body≡stage0）；下一刀 drone body≡stage0；`web-build` 仍一次 zig/Python 产核 |
...
| **M3** | ① **`compiler.ujs`**（编译器用 UJS 写）② **stage0/1/2**：stage2 字节 ≡ stage1 ③ 自举门禁脚本 | 「差不多能编」无字节一致 | **v10✓** sim body≡stage0；下一刀 drone |
...
M2 编译器 wasm 化          **✓**（compile+host+sim+ship emit 无 python3）
        ↓
M3 UJS 自举                **v10✓** sim body≡stage0 → drone → P0 余 web-build
        ↓
P1 构造迁 unisacc          可选
```
...
| 件 | 说明 |
|---|---|
| `compiler.ujs` | v9 + global setidx · f64 global 默认 · len/`[` promote list |
| `compiler.wasm` | len u32 · f64 lex **ratio**（不再 place*=0.1）· MEM 128 |
| 门禁 | **sim.ujs body≡stage0** · stage2≡stage1 |

**下一刀**：`drone.ujs` body≡stage0 → 自举编玩法核。
...
| **M3** | `compiler.ujs` 自举 | body≡stage0 → stage2≡stage1 | **v10✓** sim body≡stage0；drone 未齐 |
| **3** | 换核模板 | 按下表抄路径绿 | 骨架 ✓ |
