...
def cmd_ship(a):
    """[X-7] kit: weights + MANIFEST + VM sources + sample wasm + self-check."""
    import zipfile
    from .emit_ic_c import emit as emit_ic
    ...
    vm_c = open(os.path.join(os.path.dirname(__file__), "native", "ujs_vm.c"),
                "rb").read()
    manifest = {
        "format": "UJS-1",
        "product": "wasm_run / js2wasm / web",
        "stages": {
...
def cmd_web_build(a):
    from .emit_web import web_build
    ...
    subprocess.check_call([
        ...
        "-Wl,--export=host_mk_str", "-Wl,--export=f64_of_export",
...
""",
    ...
    from .emit_browser import emit as emit_browser
...
def cmd_js2wasm(a):
    ...
    try:
        info = js2wasm_file(a.file, a.out, i64_only=getattr(a, "i64_only", False))
...
def main(argv=None):
    ...
    wb = sp.add_parser("web-build",
                       help="emit ujs/web/ujs_rt.wasm + demos (product)")
    ...
    j.add_argument("--i64-only", action="store_true",
                   help="legacy numeric WAT path")
