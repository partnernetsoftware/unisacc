#!/bin/bash
# unisaccrun.com: one file, every target. [A-38] [S-10]
#
...
./tests/build_run.sh >/dev/null || exit 1      # regenerate unisaccrun.c
T=$(mktemp -d); trap 'rm -rf "$T"' EXIT
COM=$T/unisaccrun.com
perl -e 'alarm 900; exec @ARGV' python3 -m unisa ape unisaccrun.c --via "$UA" \
...
for f in examples/hello.c examples/fib.c tests/c/b_float.c; do
    ...
    perl -e 'alarm 120; exec @ARGV' "$COM" "$f" > "$T/got" 2>/dev/null
