...
WORDS = ["type=int", "type=void", "type=static", "return", "if", "else", "while", "for", "eof",
         ...
         "typedef", "struct", "type=long", "type=char", "type=unsigned", "type=short", "type=signed", "[", "]", "...", "type=double", "type=float", ".", "->", "sizeof", "switch", "case", "default", "enum"]
...
# double: base code DBL.  Storage only -- measured (probe p63): a double local, parameter, return value,
# va_arg(ap, double) and `*(unsigned long *)&d` are the plain 64-bit moves of a long (load64/store64,
# integer registers, no floating instruction).  Every other use converts or compares in floating point
...
def expr():
    ...
    for L in levels:
        ...
        for op in cases:
            q = P(cases[op])
            if op == "&&":
                q.call("DNO").newlab("a").vpush("a").o("  jumpz r0, ").lab("a").o("\n").call("NEXT").call(sub)
                q.call("DNO").o(NORM).vpop("a").lab("a").o(":\n").a(("LDI", "pt", 0), ("LDI", "pb", 0)).goto("LOOP%d" % L)
            elif op == "||":
                q.call("DNO").newlab("a").newlab("b").vpush("a")
                q.o("  jumpz r0, ").lab("b").o("\n  imm r0, 1\n  jump ").lab("a").o("\n").lab("b").o(":\n")
                q.call("NEXT").call(sub).call("DNO").o(NORM).vpop("a").lab("a").o(":\n").a(("LDI", "pt", 0), ("LDI", "pb", 0)).goto("LOOP%d" % L)
    ...
    for o in CASOPS:     # a op= e: address, load, push, e, op, store (measured)
        ...
        q.o(PUSH)
        vwidth(q, "pt", "pb", LD)
        q.o(PUSH).vpush("pt", "pb").call("NEXT").call("EXPR").call("DNO").call("UFLAG").vpop("pt", "pb")
    ...
    p = P("EXPR.sta")
    p.a(("ALUI", "sub", "pt", "pt", 1)).o(PUSH).vpush("pt", "pb").call("NEXT").call("EXPR").a(("COPYW", "spt", "pt"), ("COPYW", "spb", "pb")).vpop("pt", "pb").call("DMATCH").o(POP1)
    ...
    p = P("SEXPR")
    p.tok({TK_ID: "SX.id"}, ("rej", "not covered: struct value"))
    P("SX.id").a(("COPYW", "sps", "ps"), ("COPYW", "spe", "pe")).call("NEXT").tok({"(": "SX.call"}, "SX.var")
    P("SX.call").call("IDTAIL").branch({1: "SX.c1"}, "DEADSV", [("CMPI", "pt", 0)])
    P("SX.c1").branch({(1, 2): "RET"}, "DEADSV", [("CMPI", "pb", SBB)])
    p = P("SX.var")
    lookup(p, "sps", "spe")
    p.branch({1: "SX.v1"}, "DEADSV", [("CMPI", "pt", 0)])
    q = P("SX.v1")
    q.branch({(1, 2): "SX.v2"}, "DEADSV", [("CMPI", "pb", SBB)])
    q = P("SX.v2")
    addr(q, "r0")
    q.ret()
    p = P("SCOPY")    # measured: 8-byte words, then a 4-byte tail (12 = 8 + 4); other tails not measured
    p.a(("LDI", "ck", 0)).label("SC.l")
    p.a(("ALUI", "add", "t", "ck", 8)).branch({(0, 1): "SC.w8"}, "SC.r", [("CMP", "t", "csz")])
    P("SC.w8").o("  load64 r2, [r1+").num("ck").o("]\n  store64 [r0+").num("ck").o("], r2\n").a(("ALUI", "add", "ck", "ck", 8)).goto("SC.l")
    P("SC.r").a(("ALU", "sub", "t", "csz", "ck")).branch({1: "RET"}, "SC.r4", [("CMPI", "t", 0)])
    P("SC.r4").branch({1: "SC.w4"}, ("rej", "not covered: struct copy tail"), [("CMPI", "t", 4)])
    P("SC.w4").o("  .ld r2, [r1+").num("ck").o("], 4\n  .st [r0+").num("ck").o("], r2, 4\n").ret()
    p = P("MEMB")
    ...
    p.cur = dk
    p.o(PUSH).vpush("pt", "pb").call("NEXT").call("EXPR").a(("COPYW", "spt", "pt"), ("COPYW", "spb", "pb")).vpop("pt", "pb").call("DMATCH").o(POP1)
    vwidth(p, "pt", "pb", ST)
    p.ret()
    p = P("EXPR.use")
    ...
    p.call("DNO").newlab("a").newlab("b").o("  jumpz r0, ").lab("a").o("\n").vpush("a", "b").call("NEXT").call("CEXPR").expect(":")
    p.call("DNO").call("UFLAG").branch({1: "DEADU", 2: "EXPR.qu2"}, "EXPR.q2", [("CMPI", "uf", 1)])
    P("EXPR.qu2").branch({1: "DEADU"}, "EXPR.q2", [("CMPI", "uf", 2)])
    p = P("EXPR.q2")
    p.vpop("a", "b").o("  jump ").lab("b").o("\n").lab("a").o(":\n").vpush("b").call("NEXT").call("EXPR")
    p.call("DNO").call("UFLAG").branch({1: "DEADU", 2: "EXPR.qu3"}, "EXPR.q3", [("CMPI", "uf", 1)])
    ...
    P("CEXPR").call("CEXPRD").call("DNO").ret()   # a value context: a double value is not covered
    for nm, stops in (("VEXPR", (";", ",", ")")), ("CEXPRD", (",",))):
        q = P(nm)
        q.tok({TK_ID: nm + ".id"} if nm == "CEXPRD" else {TK_ID: nm + ".id", "*": nm + ".st"}, nm + ".e")
    ...
    lookup(p, "sps", "spe")
    noarr(p)
    sa, sn = p.fresh("sa"), p.fresh("sn")
    p.branch({1: sa + "0"}, sn, [("CMPI", "pt", 0)])
    P(sa + "0").branch({(1, 2): sa}, sn, [("CMPI", "pb", SBB)])
    q = P(sa)     # measured: push &s; the source's address; mov r1, r0; pop r0; copy
    addr(q, "r0")
    q.o(PUSH).vpush("pb").call("NEXT").call("SEXPR").a(("COPYW", "spb", "pb")).vpop("pb")
    q.branch({1: sa + "1"}, "DEADSV", [("CMP", "pb", "spb")])
    q = P(sa + "1")
    q.o("  mov r1, r0\n  load64 r0, [r7+0]\n  .frame -8\n").a(("ALUI", "sub", "t", "pb", SBB), ("LDX", "csz", "t", SSZ)).call("SCOPY").ret()
    p.cur = sn
    addr(p, "r0")
    p.o(PUSH).vpush("pt", "pb").call("NEXT").call("EXPR").a(("COPYW", "spt", "pt"), ("COPYW", "spb", "pb")).vpop("pt", "pb").call("DMATCH").o(POP1)
    vwidth(p, "pt", "pb", ST)
    ...
    for nm, txt in (("U.neg", "  imm r1, 0\n  sub64 r0, r1, r0\n"), ("U.not", "  imm r1, 0\n  eq r0, r0, r1\n"),
                    ("U.cpl", "  imm r1, -1\n  xor64 r0, r0, r1\n")):
        q = P(nm).call("NEXT").call("UNARY").call("DNO")
    ...
    p = P("IT.ok1")
    p.a(("LDX", "t", "v", VAR)).branch({1: "IT.vc"}, "IT.ok1v", [("CMPI", "t", 1)])
    p = P("IT.vc")
    p.vpush("sps", "spe").a(("LDI", "na", 0)).call("NEXT").tok({")": "DEADV"}, "IT.varg")
    p = P("IT.varg")
    p.vpush("na").call("EXPR").call("DNO").vpop("na").o(PUSH).a(("ALUI", "add", "na", "na", 1))
    p.tok({",": "IT.vcm", ")": "IT.vcl"}, ("rej", "not covered: argument list"))
    P("IT.vcm").call("NEXT").goto("IT.varg")
    p = P("IT.vcl")
    p.a(("LDI", "vi", 0), ("ALUI", "sub", "vj", "na", 1)).label("IT.vsw")
    p.branch({0: "IT.vs1"}, "IT.vend", [("CMP", "vi", "vj")])
    p = P("IT.vs1")
    p.a(("ALUI", "mul", "oi", "vi", 8), ("ALUI", "mul", "oj", "vj", 8))
    p.o("  load64 r2, [r7+").num("oi").o("]\n  load64 r1, [r7+").num("oj").o("]\n  store64 [r7+").num("oi").o("], r1\n  store64 [r7+").num("oj").o("], r2\n")
    p.a(("ALUI", "add", "vi", "vi", 1), ("ALUI", "sub", "vj", "vj", 1)).goto("IT.vsw")
    p = P("IT.vend")
    p.vpop("sps", "spe").a(("ALUI", "mul", "t", "na", 8)).o("  call ").a(("SPAN2", "sps", "spe")).o("\n  .frame -").num("t").o("\n")
    p.a(("INTERN", "v", "sps", "spe"), ("LDX", "pt", "v", FRD), ("LDX", "pb", "v", FRB)).call("NEXT").ret()
    g.on("DEADV", range(257), "DEAD", rej("not covered: call to a variadic function"), "r")
    p = P("IT.ok1v")
    p.a(("LDX", "t", "v", DPR)).branch({1: "DEADDP"}, "IT.ok1w", [("CMPI", "t", 1)])
    g.on("DEADDP", range(257), "DEAD", rej("not covered: call to a function with a double parameter"), "r")
    p = P("IT.ok1w")
    p.a(("LDX", "t", "v", LOC))
    ...
    p = P("IT.arg")
    p.vpush("na").call("EXPR").call("DNO").call("UFLAG").vpop("na").o(PUSH).a(("ALUI", "add", "na", "na", 1), ("COPYW", "la", "uf"))
...
def printf():
    ...
    p = P("PF.arg")
    p.vpush("na", "pb").call("NEXT").call("EXPR").call("DNO").vpop("na", "pb")
...
def stmt():
    ...
    P("S.tdd").a(("LDI", "bni", 1), ("LDX", "bsz", "v", TDB), ("LDX", "sd0", "v", TDD)).branch({(1, 2): "S.tds"}, "S.tdn", [("CMPI", "bsz", SBB)])
    P("S.tds").a(("LDI", "bni", 0)).goto("S.tdn")
    P("S.tdn").call("NEXT").goto("D.one")
    ...
    p.call("NEXT").ret()
    p = P("S.rete")
    p.branch({1: "S.rt0"}, "S.rtn", [("CMPI", "rptr", 0)])
    P("S.rt0").branch({(1, 2): "S.rts"}, "S.rtn", [("CMPI", "rbsz", SBB)])
    p = P("S.rts")
    p.call("SEXPR").expect(";").branch({1: "S.rts1"}, "DEADSV", [("CMP", "pb", "rbsz")])
    p = P("S.rts1")
    p.o("  mov r1, r0\n  .lea r0, __rv_").a(("SPAN2", "fps", "fpe")).o("\n")
    p.a(("ALUI", "sub", "t", "rbsz", SBB), ("LDX", "csz", "t", SSZ)).call("SCOPY")
    p.o("  jump R").num("rl").o("\n").call("NEXT").ret()
    p = P("S.rtn")
...
def unit():
    ...
    P("TD").a(("LDI", "tdd", 0), ("LDI", "tdb", CUNK), ("LDI", "tdw", 0), ("LDI", "tds", 0)).call("NEXT").tok(dict(TW, struct="TD.s", **{"type": "TD.wi", "type=char": "TD.wc", "type=short": "TD.ws", "type=long": "TD.wl"}), ("rej", "not covered: typedef"))
    P("TD.s").a(("LDI", "tdw", 2)).call("NEXT").tok({TK_ID: "TD.st", "{": "TD.sa"}, ("rej", "not covered: typedef"))
    P("TD.st").a(("INTERN", "tg", "ps", "pe"), ("LDX", "t", "tg", STAG)).branch({1: "TD.st0"}, "TD.st1", [("CMPI", "t", 0)])
    P("TD.st1").a(("ALUI", "add", "tds", "t", SBB)).goto("TD.st0")
    P("TD.st0").call("NEXT").tok({"{": "TD.sb"}, "TD.wx")
    P("TD.sa").a(("LDI", "tg", 0)).goto("TD.sb")
    P("TD.sb").call("SBODY").a(("ALUI", "add", "tds", "nsid", SBB)).call("NEXT").goto("TD.wx")
    P("SD").call("NEXT").tok({TK_ID: "SD.t"}, ("rej", "not covered: struct"))
    P("SD.t").a(("INTERN", "tg", "ps", "pe")).call("NEXT").tok({"{": "SD.def"}, "SD.fn")
    P("SD.def").call("SBODY").call("NEXT").expect(";").call("NEXT").goto("TOP")
    # struct TAG f(...) / struct TAG *f(...): a function of a struct (value or pointer) type; a struct global stays not covered (gtu)
    p = P("SD.fn")
    p.a(("LDX", "t", "tg", STAG)).branch({1: "DEADST"}, "SD.fn1", [("CMPI", "t", 0)])
    P("SD.fn1").a(("ALUI", "add", "bsz", "t", SBB), ("LDI", "bni", 0), ("LDI", "sd0", 0), ("LDI", "gtu", 1)).goto("FN.n1")
    ...
    P("TD.idu").a(("LDI", "tdb", CUNK)).branch({1: "TD.id3"}, "TD.ids", [("CMPI", "tds", 0)])
    P("TD.ids").a(("COPYW", "tdb", "tds")).goto("TD.id3")
    p = P("TD.id3")
    ...
    P("TOP.std").a(("LDI", "bni", 0), ("LDI", "bsz", CUNK), ("LDI", "sd0", 0), ("LDI", "gtu", 1), ("LDX", "t", "v", TDB)).branch({(1, 2): "TOP.sts"}, "FN.n", [("CMPI", "t", SBB)])
    P("TOP.sts").a(("COPYW", "bsz", "t"), ("LDX", "sd0", "v", TDD)).goto("FN.n")
    p = P("FN")
    ...
    # pointer of unknown base (measured: static void (*fa[4])(void) -> .bss g_fa 32); the
    ...
    P("FN.fpc").call("NEXT").expect("(").a(("LDI", "ad", 1)).call("NEXT").label("FN.fpl")
    p = P("FN.fpl")
    p.tok({"(": "FN.fpo", ")": "FN.fpx"}, "FN.fpe")
    P("FN.fpe").tok({"eof": "FN.fpbad"}, "FN.fpk")
    g.on("FN.fpbad", range(257), "DEAD", rej("not covered: declarator"), "r")
    P("FN.fpk").call("NEXT").goto("FN.fpl")
    P("FN.fpo").a(("ALUI", "add", "ad", "ad", 1)).goto("FN.fpk")
    P("FN.fpx").a(("ALUI", "sub", "ad", "ad", 1)).branch({1: "FN.fpd"}, "FN.fpk", [("CMPI", "ad", 0)])
    P("FN.fpd").call("NEXT").tok({";": "FN.fpg"}, ("rej", "not covered: function pointer declaration"))
    P("FN.fpg").a(("INTERN", "v", "fps", "fpe")).branch({1: "FN.gt"}, "GV", [("CMPI", "gtu", 1)])
    ...
    dp, dn = p.fresh("fd"), p.fresh("fe")
    p.branch({1: dp}, dn, [("CMPI", "ptd", 0)])
    P(dp).branch({1: dp + "x"}, dn, [("CMPI", "bsz", DBL)])
    P(dp + "x").a(("LDI", "fdp", 1)).goto(dn)
    p.cur = dn
    declare(p)
    ...
    p = P("FN.hd")
    p.a(("ALUI", "div", "np", "cur", 8), ("STX", "fv", VAR, "vfn"), ("STX", "fv", DPR, "fdp")).call("NEXT").tok({"{": "FN.body", ";": "FN.proto"}, ("rej", "not covered: declaration"))
    ...
    rv, rn = p.fresh("rv"), p.fresh("rn")
    p.branch({1: rv + "0"}, rn, [("CMPI", "rptr", 0)])
    P(rv + "0").branch({(1, 2): rv}, rn, [("CMPI", "rbsz", SBB)])
    P(rv).o(".bss __rv_").a(("SPAN2", "fps", "fpe"), ("ALUI", "sub", "t", "rbsz", SBB), ("LDX", "csz", "t", SSZ)).o(" ").num("csz").o("\n").goto(rn)
    p.cur = rn
