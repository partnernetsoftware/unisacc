...
Per file (each child bounded, 10 s; each sim run bounded by steps):
  x    : $E3REF -dump-tokens FILE   (the reference's token stream)
  ref  : $E3REF FILE -S -o -         (the reference's tape)
  sim  : exec/pp/sim.py on x         (accept(o) or reject(k); the executor is E2's, unchanged)
The dump prints every type keyword and typedef name as `type`, so it cannot
tell `int` from `char`; a file whose SOURCE spells any type word other than
`int` (or `typedef`) is counted not-covered before the delta runs (`src-type`).
Verdicts:
...
TYPEW = re.compile(rb"\b(char|short|long|unsigned|signed|void|float|double|_Bool|typedef|struct|union|enum|"
                   rb"const|volatile|static|extern|register|auto|inline|restrict|__int128|size_t|FILE)\b")
...
def main():
    ...
    for f in sys.argv[2:]:
        src = open(f, "rb").read()
        if TYPEW.search(src):
            v, k = "not-covered", "src-type"
        else:
            rc0, x, _ = sh([REF, "-dump-tokens", f])
