...
def install(E,word):
    ...
    semantic={META.index(k)+1:'META.'+k for k in ('reloc','form','gate','carry','catop','winimp','retconv','hstd','written','scr0','scr1')}
    ...
    P('META.gform').branch({1:'AFTER'},'META.winforma',[('CMP','mv','id_svc')])
    P('META.winforma').branch({1:'META.formwin'},'FAIL',[('CMP','mv','id_winapi')])
    P('META.formwin').branch({1:'AFTER'},'FAIL',[('CMPI','target_os',3)])
    ...
    P('META.svc80').branch({1:'META.set80'},'META.winapi',[('CMP','mv','id_svc80')])
    P('META.winapi').branch({1:'META.setwin'},'FAIL',[('CMP','mv','id_winapi')])
    P('META.setwin').a(('LDI','gkind',2)).goto('AFTER')
    ...
    P('EMIT.27').branch({1:'GATE.80',2:'WGATE'},'GATE.0',[('RLD','gkind')])
