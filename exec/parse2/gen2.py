...
def types():
    ...
    p = P("TSPEC")
    p.a(("LDI", "td", 0)).tok({**{w: "TS." + w for w in TWORDS}, TK_ID: "TS.id"}, bad("type"))
...
def build():
    ...
    p.tok({**{w: "FN" for w in TWORDS}, "eof": "END", "typedef": "TD", "type=static": "TOP.st", TK_ID: "TOP.id"}, bad("top-level construct"))
    ...
    p.tok(dict({")": "FN.body", "type=void": "FN.void", TK_ID: "FN.ptk"}, **{w: "FN.par" for w in TWORDS if w != "type=void"}), bad("parameter"))
    ...
    P("FN.pn").call("NEXT").tok({**{w: "FN.par" for w in TWORDS}, TK_ID: "FN.ptk"}, bad("parameter"))
    p = P("FN.body")
    ...
    p = P("STMT")
    p.tok({"{": "S.blk", "*": "S.star", **{w: "S.decl" for w in TWORDS}, "return": "S.ret", "if": "S.if", "while": "S.while", "for": "S.for", ";": "S.empty", TK_ID: "S.idq"}, "S.expr")
    ...
    P("U.par").call("NEXT").tok({**{w: "U.cast" for w in TWORDS}, TK_ID: "U.pq"}, "U.pe")
