...
| C3 | **多错误**：遇错后同步到下一个语句/声明继续，最多报 N 个 | `diag` 套件加一个含三处错误的文件，三处都报、行列都对 |
| C4 | **一组高价值警告**：非 void 函数缺 return；printf 字面量格式与实参类型不符（我们在编译期就展开 printf，类型本就已知）；整数隐式转指针；未使用变量 | 新套件 `warn`：每类一个探针，与 `cc -Wall` 在同一行报同一类；不产生 cc 不报的警告 |

**D. 库（P1）**

| # | 事项 | 完成判据 |
|---|---|---|
| D1 | **头文件**：`<errno.h>` `<float.h>` `<iso646.h>` `<time.h>`（`time` `clock`）`<signal.h>`（最小）。`<setjmp.h>` 需要后端保存/恢复 tape 寄存器，先评估 | 每个头一个探针进 `c99` 或新的 `libc` 套件，对 cc 输出一致 |
| D2 | **函数**：`qsort bsearch strtok strncat sscanf fseek ftell remove rename perror getenv atexit labs div` | 每个函数一个探针，对 cc 一致；tools 语料不退 |
...
**明确不做**：目标文件与链接器（多单元已由一个 walker 解决）；B1 之外的优化器（`-O` 各级）；训练（对照臂）；C11/C23 中 G 组之外的特性。
