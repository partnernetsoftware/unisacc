...
| 2 | 用 unisacc 能编译的 C 子集实现构造器（读 .tsv，写 UNS2），由 unisacc 自己编译 | 18 个阶段的 UNS2 与 Python 逐字节相同；构造器由 unisacc 自编译后结果也相同；每次运行不超过 60 s | **prec、reloc 的“表 → 部署权重”切片已闭合**（fa13e73）：C 构造器全域检查两条部署不变量（激活为 0/1，b1 = 1 - 约束字段数）；`construct -u` 写出的 UNS2（146 B）与同一组阶段的 `uns2.dump` 原始字节相同，prec 83 B、reloc 47 B 两节也与发布的 built.uns2 中对应的节逐字节相同（16 B 头部因阶段数、单元数不同而不比较）；用部署解码器 `uns2.load` 加 `IntNet.predict` 读回，全部键、全部头的 argmax 都等于 TSV 标签且无并列。以上在 cc 构建与 unisacc 构建下都通过，已独立复跑。缺口：不变量的违规路径没有负例覆盖；部署的 C 内核读的是自己的 MODEL 块而不是 UNS2，所以这里的往返走的是 Python 的 UNS2 解码器；uns2.py 文档写的“presence H bits”与代码（每单元 8 位计数）不符，C 写出器跟随代码。**第一片**（06be1d6，`iterate/construct/`，825 行 C）：prec、reloc 的规范转储（含商化分组、决策表、to_dict 各字段）在 cc 构建和 unisacc -O2 构建下都与 Python 逐字节相同（855 B、348 B）；原始全部键逐头核对；五种负例都被拒（`iterate/construct/check.sh`，已独立复跑）。**未做**：其余 16 个阶段（多头的 T4 共享与 pick 尚未移植，nh≠1 直接拒绝）；C 侧的 UNS2 写出器；factored 路径没有被选中过，所以还没被逐字节检验；C 读取器不检查 UTF-8 |
...
**C 构造器的阶段限制矩阵**（上限：MAXF 3 个字段，MAXH 4 个头且目前只支持单头，MAXV 每字段 62 个取值，MAXC 32 个类别，MAXOK 4,096 个原始键，MAXQ 62 个商化键；另外一个 `-u` 包最多 MAXS 8 个阶段，所以即使解除多头和位集两项，也不能一次写出全部 18 个阶段）：
