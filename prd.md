...
**S-6 自举是"够自己用"的自举，不是追平。** [A-23] 的不动点 B=C=U 是**自我复现**，unisacc.c 只需接受它自己用到的子集。实测它接受 **111/220** 语料与 **31/73** 探针（Python 前端 209/220）：没有 switch、没有 struct/union 定义、没有 `sizeof`、没有标签与 `goto`、没有初始化器，printf 只认几个转换；而且它**只出 tape**，lowering 与镜像仍是 Python。所以"它自举"准确的说法是：**unisacc.c 能把自己编成 tape**，不能独立造出自己的可执行文件。真正的 100% 自举 = 把 lowering、两个编码器、三个镜像格式也搬进 C —— 那是搬运，不是设计（`catalog` 已是唯一真源，`ckernel.py` 已会把模型与 kernel 发成 C）。这一层的仪表是 [A-29]。
...
| 6 | **自举前端追平**（由第 1 项驱动） | **进行中**：probes 31→60、corpus 111→176，词法器 80/0 完全一致，`ccrun` 58 一致 / 2 已知分歧 / 20 拒绝；已补 switch、标签与 goto、sizeof、cast、struct/union、typedef、enum、`?:`、函数式宏、去注释、printf、整数字面量、聚合初始化器、#include、变参、无符号，见 E-47..E-51 |
...
| **scope** | C 侧在与 Python 相同的 8 个点、以相同的 key 去问。**但诚实地说：scope 的答案在两侧都不决定任何东西** —— Python 自己也是问完就丢。它要么成为真正起作用的决策（比如让「绑定到哪个作用域」由它回答），要么从主张里拿掉；记为开放项 |
