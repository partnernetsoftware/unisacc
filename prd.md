...
**C 构造器的阶段限制矩阵**（上限：MAXF 3 个字段，MAXH 4 个头（多头路径已接入，仅在 tyinfo 上验证），MAXV 每字段 128 个取值（059c506 起，原为 62），MAXC 62 个类别（0723641 起，原为 32；类别集合仍是单个 long，位 ≤ 61），MAXOK 4,096 个原始键，MAXQ 1,024 个商化键（cedd477 起为 17 个 62 位字；原先的 16 个 long 的位集，原为一个 long、62 个）；MAXR 每张决策表 62 条规则；另外一个 `-u` 包最多 MAXS 8 个阶段，所以即使解除多头和位集两项，也不能一次写出全部 18 个阶段）：
...
| type | 3 | 1 | 15/19/15 | 16 | 4,275 | 3,150 | 原始键 > 4,096，商化键 > 1,024；组数 15/14/15 ≤ 62 |
