...
- 2026-09-26 (S-17), **suspected product miscompile: `*"literal"`** (found while measuring E3, reference 4b37a77). `int f(void) { return *"z"; }` must return 122, and the system cc prints 122. unisacc.com instead returns the literal's address truncated to int: `.lea r0, S0` is emitted with no `.ld`. Reproduce with `exec/parse2/probes/s35d.c`. E3 marks it not covered. That is only an isolation of the difference from the reference; it is not a language rule, and the missing load is not copied. The product fix is pending; after it, the probe's result should be 122.
