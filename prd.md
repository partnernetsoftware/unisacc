...
| 2 | 用 unisacc 能编译的 C 子集实现构造器（读 .tsv，写 UNS2），由 unisacc 自己编译 | 18 个阶段的 UNS2 与 Python 逐字节相同；构造器由 unisacc 自编译后结果也相同；每次运行不超过 60 s | **进行中**，当前实况（2026-09-25）：已移植 prec、reloc、tyinfo 三个阶段（多头路径已接入，但只在 tyinfo 这一条路径上验证过，见下方分支记录）；C 侧已有 UNS2 写出器，三个阶段的 UNS2 原始字节与 Python 参考相同，各节也与发布的 built.uns2 相同；部署语义往返走的是 Python 的 UNS2 解码器（C 的 MODEL 内核部署未验证）；部署不变量及其负例（-T bias / -T act）已覆盖；uns2.py 的格式说明已修正；只在 osx/arm64 上测过。其余 15 个阶段未移植，受限情况见下方矩阵。*历史*：第一片 06be1d6 只做了 prec、reloc 的规范转储，当时尚无 UNS2 写出器、多头未移植、不变量没有负例 |
