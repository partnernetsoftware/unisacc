...
| 2 | 用 unisacc 能编译的 C 子集实现构造器（读 .tsv，写 UNS2），由 unisacc 自己编译 | 18 个阶段的 UNS2 与 Python 逐字节相同；构造器由 unisacc 自编译后结果也相同；每次运行不超过 60 s | **进行中**，当前实况（2026-09-25）：已移植 prec、reloc、tyinfo、regmap 四个阶段（多头路径只在 tyinfo 上验证过；分解候选只在 regmap 上验证过，见下方分支记录）；C 侧已有 UNS2 写出器，三个阶段的 UNS2 原始字节与 Python 参考相同，各节也与发布的 built.uns2 相同；部署语义往返走的是 Python 的 UNS2 解码器（C 的 MODEL 内核部署未验证）；部署不变量及其负例（-T bias / -T act）已覆盖；uns2.py 的格式说明已修正；只在 osx/arm64 上测过。其余 14 个阶段未移植，受限情况见下方矩阵。*历史*：第一片 06be1d6 只做了 prec、reloc 的规范转储，当时尚无 UNS2 写出器、多头未移植、不变量没有负例 |
...
| pp | 2 | 1 | 9/2 | 4 | 18 | 8 | 无（未试） |
| lex | 2 | 1 | 12/12 | 10 | 144 | 40 | 无（未试） |
| scope | 2 | 1 | 6/5 | 7 | 30 | 24 | 无（未试） |
| regmap | 2 | 1 | 8/2 | 16 | 16 | 16 | 无（已移植，2bd6a02） |
| pfconv | 1 | 1 | 9 | 7 | 9 | 7 | 无（未试） |
| binsel | 2 | 1 | 16/2 | 23 | 32 | 32 | 无（未试） |
