...
**C 构造器的阶段限制矩阵**（上限：MAXF 3 个字段，MAXH 4 个头（多头路径已接入，仅在 tyinfo 上验证），MAXV 每字段 128 个取值（059c506 起，原为 62），MAXC 32 个类别，MAXOK 4,096 个原始键，MAXQ 62 个商化键；另外一个 `-u` 包最多 MAXS 8 个阶段，所以即使解除多头和位集两项，也不能一次写出全部 18 个阶段）：
...
| opinfo | 1 | 3 | 67 | 16 | 67 | 18 | 无（已验收，OPINFO_H；代码未改，MAXV 放宽后即在容量内） |
| enc | 3 | 1 | 73/3/2 | 5 | 438 | 8 | 无（已移植，059c506；MAXV 已放宽到 128） |
| peep | 3 | 1 | 8/17/12 | 10 | 1,632 | 528 | 商化键 > 62 |
| type | 3 | 1 | 15/19/15 | 16 | 4,275 | 3,150 | 原始键 > 4,096，商化键 > 62 |
| parse | 2 | 1 | 5/68 | 36 | 340 | 145 | 类别 > 32、商化键 > 62（取值 68 已在 MAXV 128 内） |
| irsel | 2 | 1 | 6/70 | 70 | 420 | 414 | 类别 > 32、商化键 > 62（取值 70 已在 MAXV 128 内） |
| isel | 2 | 2 | 73/2 | 94 | 146 | 138 | 类别 > 32、商化键 > 62；多头与多字段同时出现尚未验证（取值已在容量内） |
| abi | 3 | 13 | 73/3/2 | 62 | 438 | 144 | 13 个头 > MAXH、类别 > 32、商化键 > 62（取值已在容量内） |
| combo | 3 | 15 | 73/3/2 | 94 | 438 | 414 | 15 个头 > MAXH、类别 > 32、商化键 > 62（取值已在容量内） |
