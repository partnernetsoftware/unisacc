...
**第 2 步发现的 unisacc 误编译（编译器正确性，单独立项修复）：**
  1. **已确认**：三维数组用变量下标时段错误。`long a[4][5][3]; int i=2,j=3; a[i][j][1]=7;` 在 -O0 和 -O2 下都以 139 退出，cc 正常；常量下标和二维数组都正常。构造器里已改用扁平二维绕开。
  2. **报告未复现**：子代理报告 `b1[H] = -(f(uc[H]) - 1)` 在构造器里存成 6，应为 -1；按同样形状写的最小例子输出 -1，属正确结果，要在原上下文里再缩小。
