...
**C 构造器的阶段限制矩阵**（上限：MAXF 3 个字段，MAXH 4 个头（多头路径已接入，仅在 tyinfo 上验证），MAXV 每字段 128 个取值（059c506 起，原为 62），MAXC 32 个类别，MAXOK 4,096 个原始键，MAXQ 1,024 个商化键（cedd477 起为 17 个 62 位字；原先的 16 个 long 的位集，原为一个 long、62 个）；MAXR 每张决策表 62 条规则；另外一个 `-u` 包最多 MAXS 8 个阶段，所以即使解除多头和位集两项，也不能一次写出全部 18 个阶段）：
...
| parse | 2 | 1 | 5/68 | 36 | 340 | 145 | 类别 36 > 32。其余已核：商化键 145 < 1,024，取值 68 < 128，规则 35 < MAXR 62，单元 34 < MAXU 128；组数 5/29 ≤ 62 |
| irsel | 2 | 1 | 6/70 | 70 | 420 | 414 | 类别 70 > 32，**规则 75 > MAXR 62**。已核：商化键、取值在容量内，单元 75 < 128；**组数 6/69，字段 2 超过 62** |
| isel | 2 | 2 | 73/2 | 94 | 146 | 138 | 类别 94 > 32，**规则 93 > MAXR 62**；多头加多字段尚未验证。已核：商化键、取值在容量内，单元 73 < 128；**组数 69/2，字段 1 超过 62** |
| abi | 3 | 13 | 73/3/2 | 62 | 438 | 144 | 13 个头 > MAXH，类别 62 > 32，**规则 67 > MAXR 62**。已核：商化键、取值在容量内，单元 50 < 128；组数 24/3/2 ≤ 62 |
| combo | 3 | 15 | 73/3/2 | 94 | 438 | 414 | 15 个头 > MAXH，类别 94 > 32，**规则 93 > MAXR 62**。已核：商化键、取值在容量内，单元 99 < 128；**组数 69/3/2，字段 1 超过 62** |
