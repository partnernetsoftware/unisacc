...
**C 构造器的阶段限制矩阵**（上限：MAXF 3 个字段，MAXH 16 个头（abi 起，原为 4），MAXV 每字段 128 个取值（059c506 起，原为 62），MAXC 62 个类别（0723641 起，原为 32；类别集合仍是单个 long，位 ≤ 61），MAXOK 4,352 个原始键（ad0585f 起，原为 4,096），MAXQ 3,200 个商化键（ad0585f 起，QW = (MAXQ+QB-1)/QB = 52；此前为 1,024，cedd477 起为 17 个 62 位字；原先的 16 个 long 的位集，原为一个 long、62 个）；MAXR 每张决策表 62 条规则；另外一个 `-u` 包最多 MAXS 8 个阶段，所以即使解除多头和位集两项，也不能一次写出全部 18 个阶段）：
...
| irsel | 2 | 1 | 6/70 | 70 | 420 | 414 | **字段组 69 > 62**（字段 2），**类别 70 > 62**。已核：规则 75 ≤ MAXR 80，单元 75 < 128，商化键 414 ≤ 3,200，取值 70 ≤ 128 |
