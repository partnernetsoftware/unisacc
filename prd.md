...
**C 构造器的阶段限制矩阵**（上限：MAXF 3 个字段，MAXH 4 个头（多头路径已接入，仅在 tyinfo 上验证），MAXV 每字段 128 个取值（059c506 起，原为 62），MAXC 62 个类别（0723641 起，原为 32；类别集合仍是单个 long，位 ≤ 61），MAXOK 4,352 个原始键（ad0585f 起，原为 4,096），MAXQ 3,200 个商化键（ad0585f 起，QW = (MAXQ+QB-1)/QB = 52；此前为 1,024，cedd477 起为 17 个 62 位字；原先的 16 个 long 的位集，原为一个 long、62 个）；MAXR 每张决策表 62 条规则；另外一个 `-u` 包最多 MAXS 8 个阶段，所以即使解除多头和位集两项，也不能一次写出全部 18 个阶段）：
...
| abi | 3 | 13 | 73/3/2 | 62 | 438 | 144 | 13 个头 > MAXH 4，规则 67 > MAXR 62，**候选总数 270 > MAXCAND 96**（此前矩阵漏列；T4 每轮为 sysno 重新追加 64 个分解候选）。其余已核：组数 24/3/2，类别 62 正好等于 MAXC，单元峰值 105 < 128 |
