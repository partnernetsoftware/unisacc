...
| # | 事项 | 位置 | 预期 | 闭环风险 | 论点 |
|---|---|---|---|---|---|
| J1 | **稠密表 = 运行构造网络遍历全域生成**（不读 gold），`inf` 变一次查表；`--check-oracle` 保留为“表 = 网络”的逐项证明 | ckernel、`inf` | 自建 5–10%，并消除记忆缓存冲突的抖动 | 无（tape 不变） | 网络的编译形态，逐项验证相等 |
...
| J5 | 预解析 `@family.flavor`、按 cop 缓存 `bk_facts`、`pp_act` 提到轮外 | es、bk_facts、peep_round | 5–8% | 无 | 不变 |
