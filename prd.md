...
**产物账**（每个里程碑报告，压缩前与压缩后都报）：通用执行内核；OS 启动与系统调用适配；加载器与解压器；随带的 C 库；模型；模板；`.com` 总字节。编译特有的代码不能搬进模板后漏记。“几 KB”指通用执行内核。
...
**速度**：固定参考提交、相同输入、目标与优化级、相同计时边界，重复 5 次取中位数；新路径慢于参考 3 倍以上时停下来先研究。
