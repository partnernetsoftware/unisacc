...
**S-17 模型化迁移路线**（2026-09-26 定，主人：“直接按整个编译器规划迁移”）。

**目标**：切片 = 一个很小的通用执行器；编译的动作都由模型推理完成；模型在 `.com` 里只存一份，各切片共享。理论依据与未证之处见 [`research/delta-framework.md`](research/delta-framework.md)：已证的只有 T1（网络 = 表），“整个编译 = 执行器 ∘ δ*”仍是待论证的框架。本路线的每一步都产出实测，作为论文的证据。

**参考答案**：现有的手写编译器就是 δ_ref。每迁移一层，新旧两条路径的输出必须**逐字节相同**，由现有门禁（closure 618 张镜像、difftest、corpus、c99、nativeboot、bigclosure）裁判。语义正确性（T3）因此继承自现有的差分测试，不重新论证；迁移只需证明“等于旧编译器”。

**执行器**（选项 A，低层通用原语，见 delta 草稿 §3）：读字节、比较、定宽算术、栈、字典读写、输出追加、调用 δ。不含任何 C 语义。目标规模：每个 ISA 的执行器机器码 **≤ 32 KB**（未实测，E1 验证）。

| 里程碑 | 层（现有代码） | δ 的输入与动作 | 判据 | 规模（实测后填） |
|---|---|---|---|---|
| **E0** | 执行器与表格式 | — | 执行器能跑一个玩具 δ（§4 的 LL(1) 表达式文法），cc 与 unisacc 双构建；给出执行器字节数 | 待测 |
| **E1** | 词法（`front_parse.c` 的 lex，含 TOKV 最长匹配） | 状态 × 字节 → 记号动作 | 现有 lexdiff 83 文件逐记号相同；全门禁镜像不变；记录执行器、δ 大小与速度比 | 待测 |
| **E2** | 预处理（`front_pp.c`，2,294 行） | 状态 × 记号类别 × 宏表摘要 → 展开动作 | `-E` 输出与现有逐字节相同；全门禁不变 | 待测 |
| **E3** | 解析走查（`front_parse.c`，5,161 行） | 续点 × 记号类别 × 栈顶 × 符号表摘要 → 产生式与语义动作 | tape 逐字节相同；全门禁不变 | 待测 |
| **E4** | 优化（`opt.c`，897 行） | 行类别 × 活跃性摘要 → 改写 | -O1/-O2 tape 相同 | 待测 |
| **E5** | lowering 与编码（`back_lower.c`、`back_encode.c`，2,000 行） | 操作类别 × 操作数形态 → 模板与槽位 | 六目标镜像逐字节相同 | 待测 |
| **E6** | 镜像写出（`back_image.c`，453 行） | 格式 × 段类别 → 头部模板 | 六目标镜像逐字节相同 | 待测 |
| **E7** | `.com` 布局 | — | 模型只存一份；切片 = 执行器；报告每段字节数 | 待测 |

**每个里程碑的规则**：
1. 先写该层的 δ 设计（状态、字母表、动作、δ 看不到而交给执行器的东西），写进 delta 草稿；
2. 用构造得到 δ（`build-weights` 同一方法），T1 穷举；
3. 新旧路径并存，门禁比较两者；全等之后才删旧代码；
4. 报告四个数：执行器字节、δ 字节（压缩前后）、编译速度比、门禁结果。速度变慢超过 3 倍时停下来先研究，不带着回归前进。

**风险（如实列出）**：δ 的规模未估算，可能远大于现在的 53.9 KB；推理比手写代码慢；解析层带符号表与无界类型结构，不是普通下推机（delta 草稿 §6.2）。这些都由 E0/E1 的实测先回答，再决定后面各层的做法。

**与 0.0.8 的关系**：S-16 的测试项（T1–T5）照做，它们是迁移的安全网；B2（Windows 段压缩）让位给 E7。
