...
**C 构造器的阶段限制矩阵**（上限：MAXF 3 个字段，MAXH 4 个头（多头路径已接入，仅在 tyinfo 上验证），MAXV 每字段 128 个取值（059c506 起，原为 62），MAXC 32 个类别，MAXOK 4,096 个原始键，MAXQ 1,024 个商化键（peep 起为 16 个 long 的位集，原为一个 long、62 个）；MAXR 每张决策表 62 条规则；另外一个 `-u` 包最多 MAXS 8 个阶段，所以即使解除多头和位集两项，也不能一次写出全部 18 个阶段）：
...
| peep | 3 | 1 | 8/17/12 | 10 | 1,632 | 528 | 无（已移植；商化键集合改为 16 字的位集，MAXQ 1024） |
