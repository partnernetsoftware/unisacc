...
| 2 | 用 unisacc 能编译的 C 子集实现构造器（读 .tsv，写 UNS2），由 unisacc 自己编译 | 18 个阶段的 UNS2 与 Python 逐字节相同；构造器由 unisacc 自编译后结果也相同；每次运行不超过 60 s | **第一片已达**（06be1d6，`iterate/construct/`，825 行 C）：prec、reloc 的规范转储（含商化分组、决策表、to_dict 各字段）在 cc 构建和 unisacc -O2 构建下都与 Python 逐字节相同（855 B、348 B）；原始全部键逐头核对；五种负例都被拒（`iterate/construct/check.sh`，已独立复跑）。**未做**：其余 16 个阶段（多头的 T4 共享与 pick 尚未移植，nh≠1 直接拒绝）；C 侧的 UNS2 写出器；factored 路径没有被选中过，所以还没被逐字节检验；C 读取器不检查 UTF-8 |
