...
| 6 | **自举前端追平**（由第 1 项驱动） | **进行中**：probes 31→54、corpus 111→161，词法器 80/0 完全一致，`ccrun` 48 一致 / 6 已知分歧 / 26 拒绝；已补 switch、标签与 goto、sizeof、cast、struct/union、typedef、enum、`?:`、函数式宏（含可变参数）、phase 3 去注释、printf 转换、整数字面量进制、聚合初始化器（含花括号省略与二维数组），见 E-47..E-50 |
