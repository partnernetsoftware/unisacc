...
**整包写出（进行中，未合并；提交 cd3c2a6、c01e692 暂留在子代理的 worktree 分支）**：MAXS 8 → 24，超限在任何构建之前以 4 拒绝（18 个表加 7 份 prec，真实负例）。一个进程从 18 份 TSV 写出的整包 9,124 B，在 cc、unisacc、UBSan 下都与 Python 参考、以及含 16 B 头部的 built.uns2 逐字节相同（version、nStages 18、nUnits 513、段顺序、名称集合、长度都逐项核对；排序在 C 里按名称做）。三种顺序（正常、逆序、固定乱序，54 次构建）输出相同；新增 resetstage() 在每个阶段开头显式清零各全局量，README 列出每个数组“读前必写”的依据和故意保留的进程级标志。整包往返：18 个阶段、8,484 个键、20,184 个（键, 头）对全部正确。写出失败新增退出码 7，全部是真实故障：输出路径是目录、目录不存在、ulimit -f 1 下的短写、以及在 fclose 时失败；重复段以 2、超 MAXS 以 4、空组以 2 退出。**阻塞**：unisacc 构建检测不到短写与关闭失败——自带 libc 的 include/stdio.h 中，fwrite 不带缓冲、忽略 __write 的返回值、总是返回 n；fclose 就是 close，fflush 是空操作。最小复现在 ulimit -f 1 下：cc 得到 3000、0、fclose −1，unisacc 与 Python 参考 VM 都得到 3000、5000、fclose 0；参考实现与二进制一致，所以不是误编译，而是库的语义缺口。wfail 这一批因此是红的，门禁没有合并。**待定**：是否修正 include/stdio.h（fwrite 返回实际写入字节数 / sz，并处理部分写与负返回），以及在无缓冲实现下“关闭失败”该在哪一步报告。
