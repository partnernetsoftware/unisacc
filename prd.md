...
- 下一步，收益最大且输出不变：**按函数增量地跑后几轮**。只重跑上一轮有命中的函数；被改函数入口处的活跃性没有变化时，调用它的函数也不必重跑。输出逐字节不变，所以 Python twin 不用动，918 镜像检查就能验证。评审列出的其余几项（比较与跳转融合、立即数融合、下标移位、返回值往返）都要新增 tape 操作或需要活跃性信息，按实测，每项的收益估计只有几个百分点。
