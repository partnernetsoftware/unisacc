...
> **配套**：[`prd.tree.md`](prd.tree.md) 树+DAG，开工用 · [`prd.map.md`](prd.map.md) 记忆宫殿，记全局用 · `archive/` 历史版本
>
...
**这个实验在证明什么**：一个决策层由确定性神经策略驱动的 C99 编译器（对标 tinycc / `tcc -run`），支持 6 个目标，体积小到离谱。不是"AI 写代码"，恰恰相反——编译器的决策表是**学出来的产物**，比它替代的手写 switch 更小、可热插拔、形式统一。让论点成立的交付物是 §5.3 那张体积/速度表，旁边摆一个 `tcc`。
...
|---|---|---|---|---|---|---|---|---|---|
| `pp` | 前端 · 预处理 | `dir(9)×defined(2)` | 18 | 6+6 | `12→12` | `y(4)` | — | 23 | 274 |
| `lex` | 前端 · 词法 | `c(11)×peek(11)` | 121 | 6+6 | `12→12` | `y(10)` | — | 31 | 418 |
| `parse` | 前端 · 语法 | `nt(5)×tok(54)` | 270 | 8+8 | `16→16` | `y(32)` | — | 13 | 1288 |
| `type` | 前端 · 类型 | `t1(8)×op(15)×t2(8)` | 960 | 8+8+8 | `24→16` | `y(9)` | — | 17 | 801 |
| `scope` | 前端 · 作用域 | `ctx(6)×kind(5)` | 30 | 8+8 | `16→16` | `y(7)` | — | 19 | 479 |
| `irsel` | 前端 · IR 选择 | `family(5)×flavor(27)` | 135 | 8+8 | `16→16` | `y(25)` | — | 11 | 953 |
| `enc` | 后端 · 编码形式 | `op(38)×os(3)×arch(2)` | 228 | 8+8+8 | `24→16` | `y(5)` | — | 29 | 829 |
| `reloc` | 后端 · 重定位 | `kind(3)×arch(2)` | 6 | 6+6 | `12→8` | `y(3)` | — | 37 | 161 |
| `isel` | 后端 · 指令选择 | `op(38)×arch(2)` | 76 | 12+8 | `20→24→16` | `form(5),symbol(51)` | — | 3 | 2328 |
| `abi` | 后端 · 调用约定 | `op(38)×os(3)×arch(2)` | 228 | 12+8+8 | `36→32→20` | `sysno(56),arg0(18),arg1(18),arg2(18),ret(18),tls(4),gate(5)` | bilinear 8d | 5 | 4361 |
| `combo` | 后端 · 合一（替代 isel+abi） | `op(38)×os(3)×arch(2)` | 228 | 16+8+8 | `52→48→32` | `form(5),symbol(51),sysno(56),arg0(18),arg1(18),arg2(18),ret(18),tls(4),gate(5)` | factor 12d+bilinear 8d | 7 | 10053 |
...
**G-2 type** — TYS(8) × TOPS(15) × TYS(8) → TYS|illegal，**960 行**
...
**G-2a** `illegal` 占 611/960。v1 要求"训练时按 1/19 下采样 illegal 行"——**实测证伪，已废止**：降权后 type 恒定卡在 0.999，唯一丢失的行是 `(i64,&,i32)→illegal`，正是那个稀疏正例 `(i64,&,i64)→ptr` 的近邻，且全表最紧的 margin 全部落在 `&` 上。类别平衡保护的是泛化，而这里没有泛化可保护、只有记忆；饿死 64% 的定义域，只会让网络在稀疏正例旁边变瞎。**权重取 1.0，全部 960 行等权训练、等权评估。** 代码中保留 `TYPE_ILLEGAL_WEIGHT` 以复现 v1 行为。
...
| 命题 | 状态 | 依据 |
|---|---|---|
| **P-8** 任意有限积上的全函数，存在精确实现它的 kernel 权重 | **已证明**（构造式，11/11 机器验证） | §1.4 · E-19 |
| **P-3** `net ≡ gold` 可判定 | **已证明**（全域枚举，2,300 key，0 分歧） | E-5 |
...
| **P-6** `gold ≡ C99` | **本实验不声称**。已三次由实现暴露 gold 缺陷，而 acc 全程满分 | E-2 · E-15 · E-16 |
| **P-8a** `h_min(f)` 的组合刻画 | **开放** | §1.4 |
| **P-8b** 可达性分离（精确解存在但 SGD 不可达） | **有实例，待形式化** | E-18（s2：12.8× 参数仍 0.8143） |
| **P-8c** 是否存在 `O(h_min)` 构造算法 | **开放**（注意最小 DNF 为 NP-hard） | §1.4 |

**一句话**：「能不能 100%」已关闭（P-8，平凡且已构造）；「**最小的精确解有多小、能否确定性地构造**」才是开放问题（P-8a/b/c）。

自行证伪的预测：**E-2′**（1/19 降权）· **E-3′**（神经比表省空间）· **E-6**（margin 预测量化落点，降级）· **E-7**（q2 可幸存，证伪）· **E-8**（混合 dtype 收益显著，不成立）。
...
4. **训练守则是否可成体系？** E-2 给出一条反直觉规则。超拟合任务是否还有其他与统计学习相反的守则（LR 表、初始化、正则）？
