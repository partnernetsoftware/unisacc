...
**S-15 0.0.7 计划**（2026-09-24 定，承 0.0.6 的测量）。

| # | 事项 | 为什么 | 完成判据 |
|---|---|---|---|
| 1 | **regmap：tape 栈指针映射到机器栈指针** | x86 text 的 36% 是 tape 栈机经 r10 的 push/pop（每次 7 字节）。若 r7 映射到 `rsp`，push/pop 是 1–2 字节的原生指令，`call`/`ret` 也成原生（现在 `call` 平均 19 字节）。这是**改 gold 表**（regmap 阶段）的事，正是本项目的方法论；代价是 `[rsp+disp]` 需要 SIB 字节、Windows 要求 16 字节对齐、六个目标的 ABI 都要重验 | 编译器自身 x86 text 降 ≥20%；closure/bigclosure 逐字节一致；acc 全 1.000 |
| 2 | **`.com` 的 Windows 部分也压缩** | PE 占 `.com` 约 62% 且原地执行。办法：一个小 PE 存根 + **用 C 写、由 unisacc 自己编译**的 inflate 解压器，解出真正的 PE 再运行 | `.com` 降 ≥30%；Windows 两个目标实机跑通 |
| 3 | **`_Generic`（corpus 00219）** | 需要类型系统区分**指针目标的限定符**（`const int *` 不可匹配 `int *`）与 `char`/`signed char`/`unsigned char` | 00219 通过，两个前端 |
| 4 | **GNU 语句表达式（00213/00214）** | 两个 walker 都要能把块当表达式求值，含块内标签与 goto | 两个都通过 |
| 5 | **初始化器（00216）** | tcc 的初始化器酷刑测试：花括号省略、多余花括号、混合指定初始化 | 通过；corpus 220/220 |
