...
| **P-6** `gold ≡ C99` | **本实验不声称**。已九次由实现暴露缺陷（3 次在 gold、6 次在走查器），而 acc 全程满分。外部语料给出可测数字：220 例 pass 180 / wrong 0 / unsupported 35 | E-2 · E-15 · E-16 · **E-30** |
...
| **证据** | `tests/corpus.sh` → `corpus 220   pass 180   wrong 0   unsupported 35   knownfail 5   slow 0`（程序被编成本机镜像**真实执行**，不是解释）。首次运行是 `pass 124   wrong 8`，其中 8 个是**真误编译**——通过了 difftest 43/43 的编译器，在别人的代码上错了八次 |
